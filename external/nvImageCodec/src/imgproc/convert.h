...
#define DEFINE_TYPE_RANGE(type, min_, max_)           \
    template <>                                       \
    struct const_limits<type> {                       \
        static constexpr type min = min_, max = max_; \
    }
    ...
    clamp(U value, ret_type<T>) {
        return value <= static_cast<U>(min_value<T>()) ? min_value<T>() : value >= static_cast<U>(max_value<T>()) ? max_value<T>()
                                                                                                                  : static_cast<T>(value);
    }

    template <typename T, typename U>
    NVIMGCODEC_HOST_DEV constexpr std::enable_if_t<
        needs_clamp<U, T>::value && std::is_signed<U>::value && std::is_floating_point<U>::value && std::is_unsigned<T>::value,
        T>
    clamp(U value, ret_type<T>) {
        return value <= static_cast<U>(min_value<T>()) ? min_value<T>() : value >= static_cast<U>(max_value<T>()) ? max_value<T>()
                                                                                                                  : static_cast<T>(value);
    ...
    template <typename T, typename U>
    NVIMGCODEC_HOST_DEV constexpr std::enable_if_t<
        needs_clamp<U, T>::value && std::is_signed<U>::value && std::is_integral<U>::value && std::is_unsigned<T>::value,
        T>
    clamp(U value, ret_type<T>) {
        return value <= 0 ? 0 : static_cast<std::make_unsigned_t<U>>(value) >= max_value<T>() ? max_value<T>()
                                                                                              : static_cast<T>(value);
    }
    ...
    NVIMGCODEC_HOST_DEV constexpr int32_t clamp(int64_t value, ret_type<int32_t>) {
        return value < static_cast<int64_t>(min_value<int32_t>()) ? min_value<int32_t>() : value > static_cast<int64_t>(max_value<int32_t>()) ? max_value<int32_t>()
                                                                                                                                              : static_cast<int32_t>(value);
    }

    template <>
    NVIMGCODEC_HOST_DEV constexpr int32_t clamp(uint64_t value, ret_type<int32_t>) {
        return value > static_cast<uint64_t>(max_value<int32_t>()) ? max_value<int32_t>() : static_cast<int32_t>(value);
    }

    template <>
    NVIMGCODEC_HOST_DEV constexpr uint32_t clamp(int64_t value, ret_type<uint32_t>) {
        return value < 0 ? 0 : value > static_cast<int64_t>(max_value<uint32_t>()) ? max_value<uint32_t>()
                                                                                   : static_cast<uint32_t>(value);
    }

    template <>
    NVIMGCODEC_HOST_DEV constexpr uint32_t clamp(uint64_t value, ret_type<uint32_t>) {
        return value > static_cast<uint64_t>(max_value<uint32_t>()) ? max_value<uint32_t>() : static_cast<uint32_t>(value);
    }
    ...
    namespace detail {
        ...
        struct Converter<T, T> {
            static NVIMGCODEC_HOST_DEV constexpr T Convert(T value) { return value; }

            static NVIMGCODEC_HOST_DEV constexpr T ConvertSat(T value) { return value; }

            static NVIMGCODEC_HOST_DEV constexpr T ConvertNorm(T value) { return value; }

            static NVIMGCODEC_HOST_DEV constexpr T ConvertSatNorm(T value) { return value; }
        };
