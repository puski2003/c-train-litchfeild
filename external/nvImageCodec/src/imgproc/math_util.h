...
// Going through intermediate value saves us a jump
    ...
    NVIMGCODEC_HOST_DEV inline float fast_rsqrt(float x) {
#ifdef __CUDA_ARCH__
        return __frsqrt_rn(x);
#elif defined __SSE__
        // Use SSE intrinsic.
    ...
    /// @brief Calculate fast approximation of square root reciprocal.
    NVIMGCODEC_HOST_DEV inline double fast_rsqrt(double x) {
#ifdef __CUDA_ARCH__
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr float deg2rad(float deg) {
        const float d2r = M_PI / 180;
        return deg * d2r;
    }

    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr float rad2deg(float rad) {
        const float r2d = 180 / M_PI;
        return rad * r2d;
    }

    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr double deg2rad(double deg) {
        const double d2r = M_PI / 180;
        return deg * d2r;
    }

    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr double rad2deg(double rad) {
        const double r2d = 180 / M_PI;
        return rad * r2d;
    }

    /// @brief Calculates normalized sinc i.e. `sin(pi * x) / (pi * x)`
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE double sinc(double x) {
        x *= M_PI;
        if (std::abs(x) < 1e-8)
    ...
    /// @brief Calculates normalized sinc i.e. `sin(pi * x) / (pi * x)`
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE float sinc(float x) {
        x *= M_PI;
        if (std::abs(x) < 1e-5f)
