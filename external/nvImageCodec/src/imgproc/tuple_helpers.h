/*
 * SPDX-FileCopyrightText: Copyright (c) 2018-2923 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
...
namespace nvimgcodec {
    namespace detail {
        ...
        template <>
        struct build_seq_impl<0, 0> {
            using type = seq<>;
        };

        template <>
        struct build_seq_impl<0, 1> {
            using type = seq<0>;
        };

        // for compilation speed
        template <>
        struct build_seq_impl<0, 2> {
            using type = seq<0, 1>;
        };

        // for compilation speed
        template <>
        struct build_seq_impl<0, 3> {
            using type = seq<0, 1, 2>;
        };

        // for compilation speed
        template <>
        struct build_seq_impl<0, 4> {
            using type = seq<0, 1, 2, 3>;
        };

        // for compilation speed
        template <>
        struct build_seq_impl<0, 5> {
            using type = seq<0, 1, 2, 3, 4>;
        };
        ...
        constexpr auto apply_all(F&& f, Args&&... args)
            -> decltype(nvimgcodec::detail::apply(f, std::tuple_cat(as_tuple(args)...))) {
            return nvimgcodec::detail::apply(f, std::tuple_cat(as_tuple(args)...));
        }
