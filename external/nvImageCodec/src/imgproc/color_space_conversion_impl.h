...
namespace nvimgcodec {
    ...
    namespace detail {
        ...
        template <typename T>
        constexpr NVIMGCODEC_HOST_DEV std::enable_if_t<!std::is_floating_point<T>::value, double> get_max_value() {
        ...
        constexpr NVIMGCODEC_HOST_DEV float scale_factor() {
        ...
        template <typename T>
        constexpr NVIMGCODEC_HOST_DEV std::enable_if_t<std::is_integral<T>::value, float> bias_scale() {
    ...
    struct itu_r_bt_601 {
        ...
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output rgb_to_y(vec<3, Input> rgb) {
            constexpr vec3 coeffs = vec3(0.25678823529f, 0.50412941176f, 0.09790588235f) * detail::scale_factor<Input, Output>();
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output rgb_to_cb(vec<3, Input> rgb) {
            constexpr vec3 coeffs = vec3(-0.14822289945f, -0.29099278682f, 0.43921568627f) * detail::scale_factor<Input, Output>();
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output rgb_to_cr(vec<3, Input> rgb) {
            constexpr vec3 coeffs = vec3(0.43921568627f, -0.36778831435f, -0.07142737192) * detail::scale_factor<Input, Output>();
            constexpr float bias = 0.5f * detail::bias_scale<Output>();
            float y = dot(coeffs, rgb) + bias;
            return needs_clamp<Input, Output>::value ? Convert<Output>(y) : ConvertSat<Output>(y);
        }
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<3, Output> rgb_to_ycbcr(vec<3, Input> rgb) {
            return {
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output y_to_gray(Input y) {
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output gray_to_y(Input y) {
            constexpr float bias = 0.0625f * detail::bias_scale<Output>();
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<3, Output> ycbcr_to_rgb(vec<3, Input> ycbcr) {
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output ycbcr_to_gray(vec<3, Input> ycbcr) {
            return y_to_gray<Output>(ycbcr[0]);
        }

        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<3, Output> gray_to_ycbcr(Input gray) {
    ...
    struct jpeg {
        ...
        template <typename Output, typename Input>
        static NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Output gray_to_y(Input gray) {
    ...
    template <typename Output, typename Input>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE Output rgb_to_gray(vec<3, Input> rgb) {
