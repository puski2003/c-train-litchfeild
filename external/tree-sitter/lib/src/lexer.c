...
static void ts_lexer__get_lookahead(Lexer* self) {
    ...
    TSDecodeFunction decode =
        self->input.encoding == TSInputEncodingUTF8 ? ts_decode_utf8 : self->input.encoding == TSInputEncodingUTF16LE ? ts_decode_utf16_le
                                                                   : self->input.encoding == TSInputEncodingUTF16BE   ? ts_decode_utf16_be
                                                                                                                      : self->input.decode;
...
void ts_lexer_init(Lexer* self) {
    *self = (Lexer){
        ...
        .column_data = {.valid = false, .value = 0}};
