        ...
        *error_type = ts_query__parse_pattern(self, &stream, 0, false, false, &capture_quantifiers);
        ...
        // next branch. After a quantified branch matches, it loops back to the cleaned copy.
        {
            ...
            for (uint32_t i = pat_start; i < pat_end; i++) {
                ...
                if (!s->is_pass_through || !s->is_inside_alternation || s->alternative_index == NONE || s->alternative_index >= i)
                    continue;
                ...
                if (target_alt_index == NONE || target_alt_index <= target_idx || target_alt_index >= pat_end)
                    continue;
...
bool ts_query_cursor_next_capture(
    ...
    for (;;) {
        ...
        for (unsigned i = 0; i < self->finished_states.size;) {
            ...
            bool node_precedes_range = (ts_node_end_byte(node) <= self->included_range.start_byte ||
                                        point_lte(ts_node_end_point(node), self->included_range.start_point));
            bool node_follows_range = (ts_node_start_byte(node) >= self->included_range.end_byte ||
