...
static inline TSNode ts_node__next_sibling(TSNode self, bool include_anonymous) {
    ...
    while (!ts_node_is_null(node)) {
        ...
        while (ts_node_child_iterator_next(&iterator, &child)) {
            ...
            bool contains_target = is_empty ? child_start_byte < start_byte : child_start_byte <= start_byte;
...
TSNode ts_node_parent(TSNode self) {
    ...
    while (true) {
        ...
        if (next_node.id == self.id || ts_node_is_null(next_node))
            break;
        node = next_node;
    }
...
TSNode ts_node_child_with_descendant(TSNode self, TSNode descendant) {
    ...
    do {
        ...
        do {
            if (
                !ts_node_child_iterator_next(&iter, &self) || ts_node_start_byte(self) > start_byte) {
                return ts_node__null();
            }
